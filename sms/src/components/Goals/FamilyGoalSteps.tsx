...
const FamilyGoalSteps: React.FC = () => {
  ...
  const handleRelativeDataChange = (field: string, value: string) => {
  ...
  const validateRelativeData = () => {
    const isIinValid = relativeData.iin.length === 12;
    const isIinConfirmed = relativeData.iin === relativeData.confirmIin;
    const isNameValid = relativeData.fullName.trim().length > 0;
    
    setIsRelativeDataValid(isIinValid && isIinConfirmed && isNameValid);
  };
  ...
  const handleNext = () => {
    if (currentStep === 1 && isRelativeDataValid) {
      setCurrentStep(2);
    } else if (currentStep === 2 && selectedPilgrimage) {
      setIsFirstStepConfirmed(true);
      setCurrentStep(3);
    } else if (currentStep === 3 && selectedPackage) {
  ...
  const renderProgressStepText = (stepNumber: number, text: string) => {
    ...
    const isActive = (stepNumber === 1 && currentStep >= 1) ||
      (stepNumber === 2 && currentStep >= 2) ||
      (stepNumber === 3 && currentStep >= 3);

    // Завершенный шаг (текст скрыт, есть галочка)
    const isCompleted = (stepNumber === 1 && currentStep > 1) ||
      (stepNumber === 2 && currentStep > 2) ||
      (stepNumber === 3 && currentStep > 3);
            ...
            <div className="goals__form">
              <div className="form-group">
                <input
                  ...
                  onChange={(e) => handleRelativeDataChange('fullName', e.target.value)}
              ...
              <div className="form-group">
                <input
                  type="text"
                  id="iin"
                  value={relativeData.iin}
                  onChange={(e) => handleRelativeDataChange('iin', e.target.value)}
              ...
              <div className="form-group">
                <input
                  type="text"
                  id="confirmIin"
                  value={relativeData.confirmIin}
                  onChange={(e) => handleRelativeDataChange('confirmIin', e.target.value)}
                  placeholder="Повторите ИИН"
                  className="form-input"
                  maxLength={12}
                />
              </div>
  ...
  return (
    <div className="goals">
      ...
      <div className="goals__progress">
        <div className="progress-container">
          <div className="progress-steps-text">
            ...
            {renderProgressStepText(3, 'Шаг 3')}
          </div>
          <div className="progress-bars">
            <div className="progress-step">
              <div
                ...
                style={{
                  width: currentStep > 1 ? '100%' : (currentStep === 1 ? '33%' : '0%')
                }}
              />
            </div>
            <div className="progress-step">
              <div
                className="progress-step-fill"
                style={{
                  width: currentStep > 2 ? '100%' : (currentStep === 2 ? '33%' : '0%')
                }}
              />
            </div>
            <div className="progress-step">
              <div
                className="progress-step-fill"
                style={{
                  width: currentStep > 3 ? '100%' : (currentStep === 3 ? '33%' : '0%')
                }}
              />
            </div>
          </div>
      ...
      <div className="title_header">
        {currentStep === 1 ? 'Укажите данные близкого человека            ' :
          currentStep === 2 ? 'Выберите тип паломничества' :
            currentStep === 3 ? 'Выберите пакет' :
